...
module.exports.run = async (bot, msg, args) => {
    if(args[0]) {
        needle.get(`https://nekobot.xyz/api/imagegen?type=blurpify&image=${args[0]}`, function(err, res) {
    ...
    if (msg.attachments.size > 0) {
        msg.attachments.forEach(img => {
            needle.get(`https://nekobot.xyz/api/imagegen?type=blurpify&image=${img.url}`, function(err, res) {
                if (err && res.body.status != 200) {
                    msg.channel.send({
                        embed: embeds.errorEmbed('An error occured.', msg.author.avatarURL(), msg.author.tag)
                    }).then(async msg => msg.delete({timeout: 5000}));
